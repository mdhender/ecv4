...
| 6 | **Refresh-token table has no cleanup** (accepted earlier). Rows accumulate; expired/revoked tokens are never pruned. | **FIXED**: `Store.PurgeExpiredRefreshTokens` deletes rows past their expiry (revoked-but-unexpired kept for reuse detection); a background reaper sweeps on `--session-reap-interval` (default 15m, 0 disables) bound to the server context, and `POST /admin/refresh-tokens/purge` (admin) runs it on demand. `internal/store/store.go`, `internal/cli/reaper.go`, `internal/handlers/server.go`. | done (small) | resolved |
